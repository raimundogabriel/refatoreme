...
const ChatEducacional = () => {
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Olá! Sou o bot de treino. Como posso ajudar?",
      sender: 'bot'
    }
  ]);
  ...
  const sendMessage = async (e) => {
    ...
    const contexto = `pre prompt: se passe por um filosofo grego no ano de 3000 AC. formato: responder por caracteres textuais sem formatação   em seguida o prompt digitado pelo usuario`;
    ...
    try {
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: contexto + currentInput }]
        })
      });
      ...
      const botMsg = {
        id: Date.now() + 1,
        text: data.choices[0].message.content, // Aqui vem a resposta real da IA
        sender: 'bot'
      };
      ...
      setMessages(prev => [...prev, { id: Date.now(), text: "Ops, erro na conexão!", sender: 'bot' }]);
    } finally {
      setLoading(false);
    }
  };
